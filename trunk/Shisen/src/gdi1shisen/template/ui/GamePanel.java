...
package gdi1shisen.template.ui;
...
import gdi1shisen.exceptions.InvalidOperationException;
import gdi1shisen.exceptions.ParameterOutOfRangeException;

/*============================================================================*/

/**
 * Class representing a basic Shisen game field display that you can extend for
...
public abstract class GamePanel extends JPanel implements MouseListener {
	...
	protected void registerImage(String identifier, String fileName)
		...
		try
		{
			...
			File imgF = new File(f.getCanonicalPath());
			URL u = imgF.toURI().toURL();
			registerImage(identifier, u);
		}
	 ...
	 * @return The placed entity (JButton).
	 */
	protected JButton placeEntity(ImageIcon icon){
	...
	/**
	 * Neue Funktion, die benötigt wird um bei einem TileSet-Switch
	 * während eines laufenden Levels bestimmte Buttons mit korrekten
	 * Bildern zu versehen
	 * @author Thomas Lack
	 * @param btn JButton auf dem das Image erneuert werden soll
	 * @param key String der zu einem Bild in der Hashmap passt
	 */
	public void refreshImage(JButton btn, String imageIdentifier)
	{
		ImageIcon img = images.get(imageIdentifier);
		btn.setIcon(img);
	}
	...
	/**
	 * Diese Methode wird aufgerufen, wenn ein Spielstein mit Rechts angeklickt wird
	 * @param positionX x-Koordinate des Spielsteins
	 * @param positionY y-Koordinate des Spielsteins
	 */
	protected abstract void entityRightPressed(int positionX, int positionY);
	
	/**
	 * Diese Methode wird aufgerufen, wenn der Rechtsklick auf einen Spielstein endet
	 */
	protected abstract void entityRightReleased();
	...
	public void mouseClicked(MouseEvent evt) {
		//Nur bei Linksklick soll entityClicked aufgerufen werden
		if (evt.getButton() == 1)
		{
			if (!hasEntities())
				return;
			...
			for (int i = 0; i < entities.size(); i++) {
				...
				if (evt.getSource() == btn) {
					...
					// -edit- speichere objekt in clicked
					this.clicked= (JButton) evt.getSource();
	...
	/**
	 * Aufruf von entityRightPressed mit x,y Koordinaten des aktuellen Spielsteins
		...
		if (evt.getButton() == 3)
		{
			...
			for (int i = 0; i < entities.size(); i++) {
				...
				if (evt.getSource() == btn) {
					...
					// pass message along
					entityRightPressed(posX / refBtn.getWidth(), posY
							/ refBtn.getHeight());
	...
	/**
	 * Aufruf der Methode entityRightReleased falls rechter Mausbutton
	 * losgelassen wird
	 */
